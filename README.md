...
1. `business_memo_system.py` captures the topic, number of revision cycles, final decision (`approve` / `reject` / `max_cycles_reached`), and whether the final memo was grounded in retrieved evidence.
2. `evaluation_logger.py` appends this row to `evaluation_log.csv` (CSV) and can also persist it in SQLite (`memo_system.db`).
3. After logging, `calculate_ges.py` can recompute the Global Evaluation Score (GES) so you can see how the system is trending over time. The helper script `run_ges.py` prints the latest metrics with one command.
...
```bash
# 1. Unit tests (calculates GES edge cases)
pytest
